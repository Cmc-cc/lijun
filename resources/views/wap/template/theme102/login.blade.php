				...
				<div class="login-box">
					...
					<div class="login-content">
						<div class="input-box account">
							<input type="text" id="account" style="font-size: 14px;" placeholder="{{trans('lang.qingshuru')}}{{trans('lang.zanghao')}}">
						</div>
						<div class="input-box pwd">
							<input type="password" id="password" style="font-size: 14px;" placeholder="{{trans('lang.qingshuru')}}{{trans('lang.mima')}}">
						</div>
						<div class="remember_forget">
							...
							<div onclick="location.href='{{$setting->service_link}}'" class="forget_pwd remember_box_text text_288">？</div>
						</div>
...
console.log(localStorage.username)
	...
	function check_login() {
		...
		if(account.length<6){
			alert("{{trans('lang.qingshuru')}}{{trans('lang.zanghao')}}");
			return false;
		}
		if(password.length<6){
			alert("{{trans('lang.qingshuru')}}{{trans('lang.mima')}}");
			return false;
		}
		$.ajax({
			...
			success : function (data) {
				...
				if(data.status.errorCode == 0){
					...
					alert("{{trans('lang.zanghao')}}或{{trans('lang.mima')}}錯誤");
				}
			}
		})
		return false;
