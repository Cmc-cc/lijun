...
<div class="m_container">
	...
	<div class="nytop" style="position: fixed;top:0;width: 100%;">
		...
		<h2><a href="/m" style="left: .3rem;">{{trans("lang.shouye")}}</a></h2>
		<h1>{{trans("lang.czjl")}}</h1>
	</div>

	<div style="position: fixed;top:.9rem;width: 100%;height:.68rem;background: #3a3a3a;">
		<a href="/m/recharge_record"><div class="acse as">{{trans("lang.cunkuan")}}</div></a>
		<a href="/m/drawing_record"><div class="acse">{{trans("lang.tixian")}}</div></a>
		<a href="/m/game_record"><div class="acse">{{trans("lang.touzhu")}}</div></a>
		<a href="/m/user_fsjl"><div class="acse">{{trans("lang.fanshui")}}</div></a>
	...
	<div class="select-info">
        ...
        <div class="select-time" style="width:72%;margin-left: 0px;">
			<div style="width:100%;overflow: hidden;">
                <p  style="width: 16%" tid="1" @if($day==1)class="on"@endif onclick="location.href='/m/recharge_record?day=1'">{{trans("lang.jinri")}}</p>
                <p style="width: 15.5%" tid="2" @if($day==2)class="on"@endif onclick="location.href='/m/recharge_record?day=2'">2{{trans("lang.day")}}</p>
				<p style="width: 15.5%" tid="3" @if($day==3)class="on"@endif onclick="location.href='/m/recharge_record?day=3'">3{{trans("lang.day")}}</p>
				<p style="width: 15.5%" tid="5" @if($day==5)class="on"@endif onclick="location.href='/m/recharge_record?day=5'">5{{trans("lang.day")}}</p>
                <p style="width: 18%" tid="7" @if($day==7)class="on"@endif onclick="location.href='/m/recharge_record?day=7'">7{{trans("lang.day")}}</p>
                <p style="width: 19.5%" tid="30" @if($day==30)class="on"@endif onclick="location.href='/m/recharge_record?day=30'">30{{trans("lang.day")}}</p>
            </div>
        </div>
    </div>
				...
				<div class="info-list">
    			    ...
    			    <table class="Hyzx-table mt10 MMain" style="line-height: 1.5;">
                        ...
                        <tr>
                            ...
                            <th style="width:35%">{{trans("lang.jine")}}</th>
                            <th style="width:35%">{{trans("lang.zhuangtai")}}</th>
                            <th style="width:35%">{{trans("lang.shijian")}}</th>
                        </tr>
                        </thead>
                                ...
                                @foreach($data as $item)
                                    <tr class="haveRead MColor1" style="display: table-row;">
                                        ...
                                        <td style="text-align:center;">
                                            @if($item->status==1)
                                                        <strong style="color:orange;">{{trans("lang.dqr")}}</strong>
                                                        @elseif($item->status==2)
                                                        <strong style="color:green;">{{trans("lang.hkcg")}}</strong>
                                                        @else
                                                        <strong style="color:red;">{{trans("lang.hksb")}}</strong>
                                                        @endif
                                        </td>
                    ...
                    <div class="page">{{$data->appends(['day'=>request()->day])->links()}}</div>
                    @else
