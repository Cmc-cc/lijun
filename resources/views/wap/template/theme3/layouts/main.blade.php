...
<head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    ...
    <link href="{{ asset('/wap/theme3/css/app.css') }}" rel="stylesheet">
    <link href="{{ asset('/wap/theme3/css/swiper.min.css') }}" rel="stylesheet">
    <link href="{{ asset('/wap/theme3/css/messenger.css') }}" rel="stylesheet">
    <link href="{{ asset('/wap/theme3/css/messenger-theme-flat.css') }}" rel="stylesheet">
    <link href="{{ asset('/wap/theme3/css/reset.css') }}" rel="stylesheet">
    <link href="{{ asset('/wap/theme3/css/BIstyle.css') }}" rel="stylesheet">

    <script type="text/javascript" src="{{ asset('/wap/theme3/js/jquery-1.11.3.min.js') }}"></script>
    <script type="text/javascript" src="{{ asset('/wap/theme3/js/flex.js') }}"></script>
    <script src="{{ asset('/wap/theme3/js/swiper.min.js') }}"></script>
    <script src="{{ asset('/wap/theme3/js/messenger.min.js') }}"></script>
    <script src="{{ asset('/wap/theme3/js/wapMain.js') }}"></script>
    <script src="{{ asset('/wap/theme3/js/mobileJS.js') }}"></script>
    <script src="{{ asset('/wap/theme3/js/bicommon.js') }}"></script>
    <script src="{{ asset('/wap/theme3/js/wapConfig.js') }}"></script>
    <script src="{{ asset('/wap/theme3/js/CUnits.js') }}"></script>
    ...
    <main class="menu slideout-menu slideout-menu-left" style="height:calc(100% - 60px);">
        ...
        <div class="menu-group">
            <div class="menu-item" plateform="game" onclick="openPlatform('game');">
                <a href="javascript:void(0)"><span><i middle="" class="iconfont icon-youxi"></i>電子游戏</span></a>
            ...
            <div class="menu-sub-group" plateform="game" style="display:none" id="game">
                ...
                ?>
                @foreach($slot as $k => $v)
                    <div class="menu-item" onclick="window.open('{{ route('game.playGame') }}?plat_type={{$v['plat_type']}}&game_type={{$v['game_type']}}&devices=1','_blank')">
                        <span>{{$v['name']}}</span>
                    </div>
                @endforeach
            </div>
            ...
            <div class="menu-item" plateform="live" onclick="openPlatform(&#39;live&#39;);">
                <a href="javascript:void(0)"><span><i middle="" class="iconfont icon-video"></i>真人視訊</span></a>
            ...
            <div class="menu-sub-group" plateform="live" style="display:none" id="live">
                ...
                ?>
                @foreach($live as $k => $v)
                    <div class="menu-item" onclick="@if($_member) window.open('{{ route('game.playGame') }}?plat_type={{$v['plat_type']}}&game_type={{$v['game_type']}}&devices=1'); @else notify('请先登录游戏！',1); @endif" platformcode="AG" gametype="">
                        <span>{{$v['name']}}</span>
                    </div>
                @endforeach
            ...
            <div class="menu-sub-group" plateform="lottery" style="display:none" id="lottery">
                ...
                @foreach($lottery as $k => $v)
                    <div class="menu-item" onclick="@if($_member) window.open('{{ route('game.playGame') }}?plat_type={{$v['plat_type']}}&game_type={{$v['game_type']}}&game_code={{$v['game_code']}}&devices=1'); @else notify('请先登录游戏！',1); @endif" platformcode="BB" gametype="Ltlottery">
                        <span>{{$v['name']}}</span>
                    </div>
                @endforeach
            </div>
            ...
            <div class="menu-item" plateform="sport" onclick="openPlatform(&#39;sport&#39;);">
                <a href="javascript:void(0)"><span><i middle="" class="iconfont icon-zuqiu"></i>體育竞技</span></a>
            ...
            <div class="menu-sub-group" plateform="chess" style="display:none" id="chess">
                ...
                @foreach($poker as $k => $v)
                    <div class="menu-item" onclick="window.open('{{ route('game.playGame') }}?plat_type={{$v['plat_type']}}&game_type={{$v['game_type']}}&devices=1');" platformcode="MW" gametype="">
                        <span>{{$v['name']}}</span>
                    </div>
                @endforeach
            </div>
            ...
            <div class="menu-item" plateform="fish" onclick="openPlatform(&#39;fish&#39;);">
                <a href="javascript:void(0)"><span><i middle="" class="iconfont icon-xiaoyouxi"></i>捕魚游戏</span></a>
            ...
            <div class="menu-sub-group" plateform="fish" style="display:none" id="fish">
                ...
                ?>
                @foreach($fish as $k => $v)
                    <div class="menu-item" onclick="@if($_member) window.open('{{ route('game.playGame') }}?plat_type={{$v['plat_type']}}&game_type={{$v['game_type']}}&game_code={{$v['game_code']}}&devices=1'); @else notify('请先登录游戏！',1); @endif">
                        <span>{{$v['name']}}</span>
                    </div>
                @endforeach
            </div>

            <div class="menu-item" plateform="youhui" onclick="window.location.href='{{ route("wap.activity_list") }}'">
        ...
        <div class="menu-group">
            <div class="menu-item" onclick="@if($_member) window.location.href = '{{route('wap.recharge')}}'; @else notify('请先登录游戏！',1); @endif">
                <a ids="deposit" href="javascript:void(0)">
                    <span><i middle="" class="iconfont icon-chongzhi"></i>充值</span>
                </a>
            </div>
            <div class="menu-item" onclick="@if($_member) window.location.href = '{{route('wap.drawing')}}'; @else notify('请先登录游戏！',1); @endif">
                <a ids="withdraw" href="javascript:void(0)">
                    <span><i middle="" class="iconfont icon-chongzhitikuan"></i>提款</span>
                </a>
            </div>
            <div class="menu-item" onclick="@if($_member) window.location.href = '{{route('wap.transfer')}}'; @else notify('请先登录游戏！',1); @endif">
                <a ids="transfer" href="javascript:void(0)"><span><i middle="" class="iconfont icon-zhuanhuan"></i>额度转换</span></a>
            </div>
            {{--div class="menu-item" onclick="return landingslocation();">
                <a href="http://tnet.ng-demo.vip/Index/transaction">
                    <span><i middle="" class="iconfont icon-jilu"></i>交易记录</span></a>
            </div>--}}
            <div class="menu-item" onclick="@if($_member) window.location.href = '{{route('wap.game_record')}}'; @else notify('请先登录游戏！',1); @endif">
                <a ids="history" href="javascript:void(0)">
                    <span><i middle="" class="iconfont icon-jilu"></i> 游戏记录</span></a>
            </div>
            @if($_system_config->is_fs==1)
                <div class="menu-item" onclick="@if($_member) window.location.href = '{{route('wap.user_fs')}}'; @else notify('请先登录游戏！',1); @endif">
...
{{--                            <div data-v-5ef742d8="" class="redPackerContentTop"><img data-v-5ef742d8="" src="./1_files/redHead.png" alt=""></div>--}}
    ...
    <div class="tabbar">
        <div class="tabbar-item">
            <a href="{{route('wap.index')}}" ids="shouye">
                <img src="{{ asset('/wap/theme3/images/icon-shouye.png') }}" alt="home">
                <p>首页</p>
            </a>
        </div>
        <div class="tabbar-item">
            <a href="{{ $_system_config->service_link }}" ids="kefu">
                <img src="{{ asset('/wap/theme3/images/icon-kefu.png') }}" alt="home">
                <p>客服</p>
            </a>
        </div>
        <div class="tabbar-item">
            <a href="javascript:void(0)" ids="deposit" onclick="@if($_member) window.location = '{{route('wap.recharge')}}' @else notify('请先登录游戏！',1); @endif">
                <img src="{{ asset('/wap/theme3/images/icon-chongzhi.png') }}" alt="pay">
                <p>充值</p>
            </a>
        </div>
        <div class="tabbar-item">
            <a href="javascript:void(0)" ids="withdraw" onclick="@if($_member) window.location = '{{route('wap.drawing')}}'; @else notify('请先登录游戏！',1); @endif">
                <img src="{{ asset('/wap/theme3/images/icon-tikuan.png') }}" alt="draw">
                <p>提款</p>
            </a>
        </div>
        <div class="tabbar-item">
            @if (Auth::guard('member')->guest())
                <a href="{{ route('wap.login') }}" ids="login">
                    <img src="{{ asset('/wap/theme3/images/icon-account.png') }}" alt="order">
                    <p>登录/注册</p>
                </a>
            @else
                <a href="{{ route('wap.nav') }}" ids="info">
                    <img src="{{ asset('/wap/theme3/images/icon-account.png') }}" alt="order">
                    <p>个人中心</p>
                </a>
            @endif
