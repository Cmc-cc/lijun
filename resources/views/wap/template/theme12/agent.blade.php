                ...
                <div class="@if($_member -> is_daili == 1) pull-left @else pull-right @endif">
                    @if ($_member->is_daili == 1)
                        <span id="url">@if($_member->agent_uri)
                            ...
                            @endif</span>
                        <span class="btn" onclick="tapCopy('url')" data-clipboard-target="#url" style="color: #e4393c;padding: 0px 17px;">复制</span>
                    @else
                        <?php
                        $apply = '';
                        ...
                        @elseif($apply && $apply->status == 2)
                            ...
                            <a href="{{ route('wap.agent_apply') }}"  style="width: 120%;font-size: 10px;background: rgb(66,96,132) !important" class="submit_btn text-center">重新申请</a>
                        @elseif($apply && $apply->status == 1)
                            <span id="url1">{{ route('web.register_one').'?i_code='.$item->invite_code }}</span>
                            <span class="btn" onclick="copyArticle1()" data-clipboard-target="#url" style="color: #e4393c;padding: 3px;">复制</span>
                        @else
                            <span style="color: red;">您还不是代理</span>
                            <a href="{{ route('wap.agent_apply') }}" style="background: rgb(66,96,132) !important" class="submit_btn text-center">立即申请</a>
                @endif
                @endif
            </dd>
            <dd>
