...
class PayController extends WebBaseController
{
    ...
    public function ajax_pay(Request $request)
    {
        ...
        // if($request->get('xiaoshu') < 10 && $request->get('xiaoshu')!=0){
        //     return responseWrong('禁止修改小数点');
        // }
        $pay_amount = $request->get('amount');
        // $pay_amount = sprintf('%.2f',$amount);    //交易金额
        $pay_type =  $request->get('paytype',5);
        $account  =  $request->get('account','');
        $bankId =  $request->get('bankId',0);
        $payment_type = $request->get('img');
        $img = $request->get('img');
        $fanli = $request->get('fanli');
        ...
        if($pay_type==3 || $pay_type==4){
            $pay_amount = $pay_amount*8;
        }
        
        if($pay_type=='usdt'){
            if(@$data['usdt_type']=='trc'){
                $pay_type = 3;
            }elseif(@$data['usdt_type']){
                $pay_type = 4;
            }
                ...
                $pay_type = 0;
                //查看银行卡号
                $bank = Db::table('bank_cards')->where('id',$data['banksss'])->first();
                $account = Base::$BANK_TYPE[$bank->bank_id].'/'.$bank->card_no;

        }else{
            $pay_amount = $pay_amount.'.'.$data['xiaoshu'];
        }
...
//                        'after_money' => $mod->member->money + $data['money'],
    ...
    public function notify(Request $request)
    {
        ...
        $get_array = [
            'signatureMethod' 	=> $request->get('signatureMethod'),
        ...
        $jz = new JzService('primary');
        //获得加密数据
        $req_signature = $jz->createSign($signArray,$this->api_key);

        if($req_signature === $signature) {
            if($data_array['status'] == 3) {
                //再次查询订单确保状态
                $res_data['orderId'] = $data_array['orderId'];
                $order = $jz->queryOrder($res_data);
                if($order['data']['status'] == 3) {

                    $mod = PayRecord::where('order_no', $order['data']["jOrderId"])->first();

                    if ($mod)
                    {
                        $member = Member::find($mod->member_id);
                        $recharge = Recharge::where('bill_no', $order['data']["jOrderId"])->first();

                        $mod->update([
                            //'status' => $data["status"],
                            'after_request_result' => json_encode($order)
                        ]);
