...
class MemberOfflineController extends DailiBaseController
{
    public function index(Request $request)
    {
        ...
        if ($request->has('is_daili'))
        {
            $status = $request->get('is_daili');
            $mod = $mod->where('is_daili', $is_daili);
        }
        ...
        return view('daili.member_offline.index', compact('data', 'name', 'status','is_daili'));
    }
