...
class IndexController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($_SERVER['HTTP_HOST'] != 'test.cs88.bet'){
        ...
        if($request->has('preview')){
            $data = Template::where('jump_name',$preview)->where('client_type',1)->first();
...
//            ]);
    ...
    public function deleteimg(){
        $data = Db::table('recharge')->where('is_delete',0)->whereNotNull('img')->where('created_at','<=',date('Y-m-d H:i:s',time()-86400*15))->take(100)->select('id','img')->get();
        if($data){
            foreach ($data as $k=>$v){
                Db::table('recharge')->where('id',$v->id)->update(['is_delete'=>1]);
                @unlink('/www/wwwroot/php/public'.$v->img);
            }
        }
    }
    public function gamelist(Request $request)
    {
        ...
        $server = new LEYUService();
        $res = $server->gamelist($api_code);

        $gamelist = json_decode($res,true);
        $gamelist = $gamelist['Data']['gamelist'];
        // dump($gamelist);die;
// var_dump($gamelist);die;
        $member = auth('member')->user();
        $gamelist = array_reverse($gamelist);

        return view('web.gamelist', compact('gamelist','api_code','member','devices'));
    }   
       ...
       public function about(Request $request)
    {
        ...
        $list = $mod->where('on_line', 1)->orderBy('sort', 'asc')->orderBy('created_at', 'desc')->get();
        return view('web.about', compact('list','data'));
    }
