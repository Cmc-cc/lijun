...
class ReportController extends AdminBaseController
{
    public function index(Request $request)
    {
        ...
        $name = $member_id = $start_at = $end_at= '';
        $rechargeMap = [];
        $drawingMap = [];
        $dividendMap = [];
        $recordMap = [];
        $members = Db::table('members')->where('is_daili', 1)->where('top_id',0)->get();
        // if(!$request->has('start_at') && !$request->has('end_at')){
        //     if ($request->has('member_id')){
        //         $member_id = $request->get('member_id');
        //         $name = Db::table('members')->where('id',$member_id)->value('name');
        //         $data = Db::table('report_data')->where('name',$name)->orderBy('created_at', 'desc')->paginate(config('admin.page-size'));
        //     }else{
        //         $data = Db::table('report_data')->orderBy('created_at', 'desc')->paginate(config('admin.page-size'));
        //     }
        //     return view('admin.report.index', compact('data', 'member_id', 'start_at', 'end_at','members'));
        // }
        ...
        if ($request->has('end_at'))
        {
            ...
            $recordMap[] = ['created_at', '<=', $end_at];
            //$mod = $mod->where('created_at', '<=',$end_at);
        }
        if ($request->has('member_id'))
        {
            $member_id = $request->get('member_id');
            $data = Db::table('members')->where('is_daili', 1)->where('top_id',0)->where('id',$member_id)->orderBy('created_at', 'desc')->paginate(config('admin.page-size'));
        }else{
        ...
        if($data){
            foreach ($data as &$v){
                ...
                // if(count($recordMap) >0){
                //     $v->yx = Db::table('game_record')->where($recordMap)->whereIn('member_id',$allteam)->sum('validBetAmount');
                //     $v->ztz = Db::table('game_record')->where($recordMap)->whereIn('member_id',$allteam)->sum('betAmount');
                //     $v->sy = Db::table('game_record')->where($recordMap)->whereIn('member_id',$allteam)->sum('netAmount');
                // }else{
                //     $v->yx = Db::table('game_record')->whereIn('member_id',$allteam)->sum('validBetAmount');
                //     $v->ztz = Db::table('game_record')->whereIn('member_id',$allteam)->sum('betAmount');
                //     $v->sy = Db::table('game_record')->whereIn('member_id',$allteam)->sum('netAmount');
                // }
    ...
    public function index_dingshi(Request $request)
    {
        ...
        $data = Db::table('members')->where('is_daili', 1)->where('top_id',0)->get();
        if($data){
            foreach ($data as &$v){
                // $team = $this->team_all_account_search($v->id,1);
                $allteam = $this->team_all_account_search($v->id,10);
        ...
        DB::select("truncate table report_data");
        foreach ($data as &$v){
            array_push($arr,[
                'teamAllCount' => $v->teamAllCount,
                'id' => $v->id,
                'name' => $v->name,
                'rechargeTotal' => $v->rechargeTotal,
                'dividendTotal' => $v->dividendTotal,
                'drawingTotal' => $v->drawingTotal,
                'yingkui' => $v->yingkui,
                // 'sy' => $v->sy,
                // 'yx' => $v->yx,
                // 'ztz' => $v->ztz,
                'agent_fencheng' => $v->agent_fencheng,
    ...
    public function team(Request $request){
        ...
        if($members && $team){
            foreach ($members as &$item){
                ...
                // if(count($recordMap) >0){
                //     $item->yx = Db::table('game_record')->where($recordMap)->whereIn('member_id',$team)->sum('validBetAmount');
                //     $item->ztz = Db::table('game_record')->where($recordMap)->whereIn('member_id',$team)->sum('betAmount');
                //     $item->sy = Db::table('game_record')->where($recordMap)->whereIn('member_id',$team)->sum('netAmount');
                // }else{
                //     $item->yx = Db::table('game_record')->whereIn('member_id',$team)->sum('validBetAmount');
                //     $item->ztz = Db::table('game_record')->whereIn('member_id',$team)->sum('betAmount');
                //     $item->sy = Db::table('game_record')->whereIn('member_id',$team)->sum('netAmount');
                // }
