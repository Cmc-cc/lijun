...
class RechargeController extends Controller
{
    ...
    {
        $mod = new Recharge();
        $account = $status = $payment_type = $name = $start_at = $end_at = $updated_start_at = $updated_end_at = '';
        if ($request->has('name'))
        {
            $name = $request->get('name');
            $m_list = Member::where('name', 'LIKE', "%$name%")->pluck('id');
        ...
        $total_recharge = $mod->sum('money');
        $total_diff_money = $mod->sum('diff_money');
        return view('admin.recharge.reindex', compact('data', 'status', 'payment_type', 'total_recharge', 'total_diff_money', 'name','account', 'start_at', 'end_at','updated_start_at','updated_end_at'));
    }
    ...
    //         if($data->diff_money > 2000){
    //             $data->diff_money = 2000;
    //         }
    //         $data->ml_money += $data->diff_money * $mod->rc_ml_percent ;
    //     }
    ...
    //             $data->diff_money = 2000;
    //         }
    //         $data->ml_money += $data->diff_money * $mod->hongli_ml_percent ;
    //     }
    ...
    public function show($id)
    {
        ...
        //     $data->diff_money = $mod->new_point;
        // }
        $data->ml_money = $data->money * $mod->rc_ml_percent;
    ...
    public function confirm(Request $request, $id)
    {
        ...
        //加锁
        $fp = fopen(base_path()."/public/recharge/".$id.".txt", "w+");
...
//                        'after_money' => $mod->member->money + $data['money'],
                ...
                if($member->is_new==1 && $mod->fanli==2){
                    Db::table('members')->where('id',$mod->member_id)->update(['is_new'=> 0]);
                    // Db::table('members')->where('id',$mod->member_id)->update(['is_tongzhi'=>0]);
                }
                if($member->is_new2==1 && $mod->fanli==3){
                    Db::table('members')->where('id',$mod->member_id)->update(['is_new2'=> 0]);
                    // Db::table('members')->where('id',$mod->member_id)->update(['is_tongzhi'=>0]);
                }
                // if($member->money <= 20){
