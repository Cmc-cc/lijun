...
class ActivityApplyController extends Controller
{
    ...
    public function show($id)
    {
        ...
        if($data->activity->type == 3){
            $recharge_money = Recharge::where('member_id', $data->member->id)->where('status', 2)->where('created_at', '>=', $data->activity->start_at)->where('created_at', '<=', $data->activity->end_at)->orderBy('created_at', 'desc')->first();
            if($recharge_money){
                $givemoney = $recharge_money->money * $data->activity->rate/100;
            }else{
                $givemoney = 0;
            }
        ...
        return view('admin.activity_apply.show', compact('data', 'givemoney','last'));
    ...
    public function confirm(Request $request, $id)
    {
        ...
        $ml_money = $mod->member->ml_money + $request->get('ml_money');
        $member_mod = Member::findOrFail($mod->member_id);
        MemberMlLog::addLog( $mod->member_id,$request->get('ml_money'),$member_mod->ml_money,3);
        
        MemberMoneyLog::addLog($mod->member_id, $money, $member_mod->money, 22, 'hd');
