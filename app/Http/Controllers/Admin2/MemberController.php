        ...
        //转账类型
            ...
            if($member_api) {
                ...
                // if($amount>0) {
                    ...
                    $result = (new LEYUService())->withdrawal($api->api_name,$user,'123456',$res_b['Data']['balance'],2);
                    $res = json_decode($result,true);
                    if (is_array($res) && $res['Code'] == '0')
                    {
                        if($api->api_name=='KY_HK'){
                            Db::table('members')->where('name',$user)->increment('money' , $res_b['Data']['balance'] / 2);
                        }else{
                            Db::table('members')->where('name',$user)->increment('money' , $res_b['Data']['balance']);
                        }
