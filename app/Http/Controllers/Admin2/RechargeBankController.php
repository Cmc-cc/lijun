...
//                        'after_money' => $mod->member->money + $data['money'],
    ...
    public function change_log(Request $request)
    {
        ...
        if($where){
            $data = Db::table('daili_change_log')->where($where)->orderBy('created_at', 'desc')->paginate(config('admin.page-size'));
        }else{
            $data = Db::table('daili_change_log')->orderBy('created_at', 'desc')->paginate(config('admin.page-size'));
        }
        return view('admin.recharge.change_log', compact('data', 'name', 'start_at', 'end_at'));
    }
